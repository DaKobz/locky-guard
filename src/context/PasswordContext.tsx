...
interface PasswordContextProps {
  ...
  updatePasswords: (passwords: PasswordEntry[]) => void;
}
...
export const PasswordProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Méthode pour restaurer tous les mots de passe à partir d'une sauvegarde
  const updatePasswords = (passwordsData: PasswordEntry[]) => {
    // Vérifier la limite pour les utilisateurs non Pro
    if (!isUserPro && passwordsData.length > FREE_USER_PASSWORD_LIMIT) {
      toast.warning(t("password.limit.partial_restore") || "Limite de mots de passe atteinte. Seuls les premiers mots de passe ont été restaurés. Passez à la version Pro pour tout restaurer.");
      // Limiter le nombre de mots de passe restaurés pour les utilisateurs gratuits
      setPasswords(passwordsData.slice(0, FREE_USER_PASSWORD_LIMIT));
    } else {
      // Restaurer tous les mots de passe pour les utilisateurs Pro
